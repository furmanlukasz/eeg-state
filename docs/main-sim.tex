...
\begin{itemize}
    \item Coupled Stuart--Landau oscillator simulation with configurable topologies
    \item Flow field estimation and trajectory metric computation
    \item Discriminability analysis with ANOVA and effect sizes
    \item Scripts to reproduce all figures from this paper
\end{itemize}

\noindent Installation and usage:
\begin{verbatim}
# Clone and install
git clone https://github.com/furmanlukasz/flowprint
cd flowprint && uv sync

# Reproduce figures
uv run python examples/reproduce_figures.py
\end{verbatim}
...
\begin{table}[h]
\centering
\caption{\textbf{Software dependencies and versions.}}
\label{tab:software}
\small
\begin{tabular}{ll}
\toprule

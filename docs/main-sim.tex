...
Metrics are computed on non-overlapping windows (50 samples $\approx$ 0.2~s after 4$\times$ compression) to reduce short-timescale autocorrelation and provide approximately independent windows for descriptive discriminability analysis. Effect sizes ($\eta^2$) are reported as primary summaries rather than $p$-values alone.

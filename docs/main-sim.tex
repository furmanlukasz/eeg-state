...
The autoencoder serves as a nonlinear coordinate transformation, not a model of the underlying generative process; dynamical quantities are computed \emph{post hoc} and are not optimized during training (see Methods).
...
The Stuart--Landau equation describes the normal form of a Hopf bifurcation and provides a canonical model for limit-cycle oscillators:
\begin{equation}
...
Let $\mathbf{x}(t) \in \mathbb{R}^{C}$ denote the multichannel signal. After bandpass filtering (2--48~Hz, zero-phase FIR filter), the analytic signal for each channel is obtained via the Hilbert transform applied to the continuous signal, yielding instantaneous phase $\phi_c(t)$ and amplitude $a_c(t)$ \citep{Boashash1992}.
...
\begin{itemize}
    ...
    \item \textbf{Visualization embedding} $\mathbf{y}(t) \in \mathbb{R}^{d}$: a low-dimensional projection of $\mathbf{h}(t)$ for plotting and pooled analysis (typically $d=2$), obtained via UMAP.
...
\end{equation}
where $\eta^2 > 0.14$ indicates a large effect, $0.06$--$0.14$ medium, and $< 0.06$ small.

Metrics are computed on non-overlapping windows (50 samples $\approx$ 0.2~s after 4$\times$ compression) to ensure independent observations for statistical testing.
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/paper3/fig_analysis_main.png}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/paper3/fig_flow_fields.png}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/paper3/fig_discriminability.png}
...
Figure~\ref{fig:kinetic_energy} presents the kinetic energy analysis. Panel~A shows the energy time series colored by regime, revealing that regime structure is visible as changes in variability and burstiness, not just mean level---consistent with metastable dynamics featuring episodes of high activity embedded in quieter motion. Panel~B displays the per-regime energy distributions, showing systematic shifts with heavy tails indicating intermittent bursts. Panel~C presents the energy landscape on the 2D embedding, revealing that some regions of the manifold are energetically more demanding to traverse than others. Panel~D summarizes mean energy and coefficient of variation by regime.

\begin{figure*}[t]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/paper3/fig_kinetic_energy.png}
...
\textbf{Metric} & \textbf{Global} & \textbf{Cluster} & \textbf{Sparse} & \textbf{Ring} \\
\midrule
Mean $E_k$ & 0.024 & 0.022 & 0.027 & 0.033 \\
CV & 0.78 & 0.72 & 0.79 & 0.84 \\
Tail index & 0.16 & 0.15 & 0.15 & 0.16 \\
\bottomrule
\end{tabular}
\vspace{0.5em}

\footnotesize{Discriminability: $F = 17.7$, $p = 3.3 \times 10^{-10}$, $\eta^2 = 0.21$ (large).}
\end{table}

Notably, the kinetic energy ordering (ring $>$ sparse $>$ global $>$ cluster) differs from the speed ordering (global $>$ cluster $>$ sparse $>$ ring), indicating that these metrics capture complementary aspects of dynamical organization. Speed reflects average motion rate, while kinetic energy---being quadratic in velocity---amplifies contributions from high-velocity excursions, revealing intermittent burst structure that mean speed obscures.
...
\begin{enumerate}
    \item \textbf{Window-based statistical testing} using non-overlapping segments ensures independent observations for ANOVA.
...
The coupled oscillator network, while providing ground-truth regime structure, is a simplified model. Real-world systems may exhibit more complex dynamics, non-stationary noise, and regime structure not captured by topology-switching alone. Empirical validation on neural, physiological, or physical data is essential.
...
Different metrics show different sensitivity profiles. The weak discriminability of tortuosity for topology contrasts does not mean tortuosity is uninformative in general---it may be highly sensitive to other types of regime differences (e.g., stability contrasts).
